package com.ambrospace.authserver.config;
...
import com.ambrospace.authserver.core.AppUserDetails;
import com.ambrospace.authserver.entity.AppUser;
import com.ambrospace.authserver.service.AppUserService;
...
public class TcktsAuthenticationProvider implements AuthenticationProvider {
    ...
    public TcktsAuthenticationProvider(OAuth2AuthorizationService authorizationService,
                                       OAuth2TokenGenerator<? extends OAuth2Token> tokenGenerator, AppUserService userDetailsService,
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        TcktsAuthenticationToken customCodeGrantAuthentication = (TcktsAuthenticationToken) authentication;
        ...
        try {
            user = appUserService.loadUserByUsername(username);
...
//        if (clientPrincipal.getPrincipal() instanceof User u) {
        additionalClaims.put("uid", user.getUserId());
//        }

        if (generatedAccessToken instanceof ClaimAccessor) {

            Map<String, Object> existingClaims = new HashMap<>(((ClaimAccessor) generatedAccessToken).getClaims());

            existingClaims.putAll(additionalClaims);

            authorizationBuilder.token(accessToken,
                    (metadata) -> metadata.put(OAuth2Authorization.Token.CLAIMS_METADATA_NAME, existingClaims));
...
//                            ((ClaimAccessor) generatedAccessToken).getClaims()));
            ...
            authorizationBuilder.attribute("additionalClaims", additionalClaims);
        }
    ...
    public boolean supports(Class<?> authentication) {
        return TcktsAuthenticationToken.class.isAssignableFrom(authentication);
    }
